...
// Console Container Background: #1A1A1A (Slightly lighter dark gray, like RightComponent on Home)

const MainContainer = styled.div`
  display: grid;
  // ⬅️ THEME CHANGE 1: Set the background of the main area to deep black
...
const Consoles = styled.div`
  ...
  grid-template-columns: 1fr;
  // ⬅️ THEME CHANGE 2: Set the console container background to a slightly lighter dark gray
...
const Playground = () => {
    ...
    const postSubmission = async (language_id, source_code, stdin) => {
        const options = {
            ...
            headers: {
                ...
                'Content-Type': 'application/json',
                // ⚠️ NOTE: It's best practice not to hardcode API keys directly in the front-end.
    ...
    const getOutput = async (token) => {
        ...
        if (res.data.status_id <= 2) {
            // Recursive call is missing 'data' property from original API call structure, 
            // but assuming the original structure was correct:
            const res2 = await getOutput(token);
            // This line looks incorrect in the original logic, assuming you want to return the result:
            // return res2.data; 
            return res2; // Return the full response object from the recursive call
        }
        return res.data;
    }
